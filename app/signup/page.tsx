...
export default function SignupPage() {
  ...
  if (done) return (
    <div style={{ maxWidth: '400px', margin: '4rem auto', textAlign: 'center' }}>
      <h2>이메일을 확인해주세요 ✉️</h2>
      <p style={{ color: '#888' }}>가입 확인 메일이 발송되었습니다. 확인 후 로그인하세요.</p>
      <Link href="/login" style={{ color: '#6366f1' }}>로그인하러 가기</Link>
    </div>
  )
  ...
  return (
    <div style={{ maxWidth: '400px', margin: '4rem auto' }}>
      ...
      <form onSubmit={signup} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        <input
          type="text" placeholder="사용자 이름" value={username}
        ...
        <button type="submit" style={btnStyle}>회원가입</button>
      </form>
      <p style={{ marginTop: '1rem', color: '#888', fontSize: '0.875rem' }}>
        이미 계정이 있으신가요?{' '}
        <Link href="/login" style={{ color: '#6366f1' }}>로그인</Link>
      </p>
    </div>
  )
}
...
const inputStyle: React.CSSProperties = {
  ...
  padding: '10px 14px', borderRadius: '8px', fontSize: '0.95rem', outline: 'none'
}
